/**
 * O-Shape Match Game
 * Match the target O-shape outline with lips
 */
...
const MATCH_THRESHOLD = 0.15; // Must be within ±0.15 of target
const MATCH_HOLD_MS = 500; // Must hold match for 500ms
const STABILITY_MS = 300;
const DEFAULT_TTS_RATE = 0.75;

// Target roundness values for different rounds
const TARGET_ROUNDNESS = [0.7, 0.8, 0.9, 0.75, 0.85];
...
export function OShapeMatchGame({ onBack, onComplete, requiredRounds = TOTAL_ROUNDS }: Props) {
  ...
  const [targetRoundness, setTargetRoundness] = useState(0.7);
  ...
  useEffect(() => {
    ...
    if (stableRoundnessStateRef.current?.value === smoothed) {
      if (now - stableRoundnessStateRef.current.since >= STABILITY_MS) {
        ...
        if (isOpen) {
          // Check if roundness matches target
          const distance = Math.abs(smoothed - targetRoundness);
  ...
  }, [landmarks, ratio, isDetecting, gameState, targetRoundness, targetGlow]);

  // Start calibration
  const startCalibration = useCallback(() => {
    setGameState('calibration');
    setCurrentRoundness(0);
    const target = TARGET_ROUNDNESS[(currentRound - 1) % TARGET_ROUNDNESS.length];
    setTargetRoundness(target);
    setMatches(0);
    setTimeElapsed(0);
    ...
    if (currentRound === 1) {
      speak(
        'Welcome to O-Shape Match! Match the target O-shape with your lips. ' +
        'Round your lips to match the circle! Show your face to the camera to start!'
      );
    } else {
      speak(`Round ${currentRound}! Match the target shape!`);
    }
  }, [currentRound, targetGlow]);
  ...
  const startCountdown = useCallback(() => {
    ...
    countdownRef.current = setInterval(() => {
      setCountdown(prev => {
        ...
        speak(prev - 1 === 0 ? 'Go! Match the shape!' : String(prev - 1));
        return prev - 1;
      });
    }, 1000);
  }, []);
  ...
  const finishGame = useCallback(async () => {
    ...
    speak(`Amazing! You matched the shape ${totalMatches} times across all rounds!`);
    ...
    try {
      ...
      const result = await logGameAndAward({
        ...
        accuracy,
        xpAwarded,
        skillTags: ['lip-rounding', 'oral-motor', 'o-sound', 'shape-matching'],
        meta: {
          totalRounds: requiredRounds,
          totalMatches,
          totalStars,
  ...
  const targetSize = Math.min(screenWidth, screenHeight) * 0.25;
      ...
      {gameState === 'playing' && (
        <View style={styles.gameArea}>
          ...
          <View style={[styles.targetContainer, {
            ...
            left: screenWidth / 2 - targetSize / 2,
          }]}>
            ...
            {/* Glow overlay using Animated.View */}
            <Animated.View
              style={[
                StyleSheet.absoluteFill,
                {
                  ...
                  borderWidth: 4,
                  borderColor: '#4CAF50',
                  opacity: targetGlow.interpolate({
                    inputRange: [0, 1, 1.5],
                    outputRange: [0, 0.8, 0],
                  }),
                  transform: [
                    {
                      scale: targetGlow.interpolate({
                        inputRange: [0, 1, 1.5],
            ...
            <Text style={styles.targetLabel}>
              Target: {Math.round(targetRoundness * 100)}%
            </Text>
          </View>
          ...
          <View style={styles.currentIndicator}>
            <Text style={styles.currentLabel}>
              Your: {Math.round(currentRoundness * 100)}%
            </Text>
              ...
              <View
                style={[
                  styles.roundnessFill,
                  {
                    ...
                    backgroundColor: Math.abs(currentRoundness - targetRoundness) < MATCH_THRESHOLD
                      ? '#4CAF50'
                      : '#FFC107',
                  },
                ]}
