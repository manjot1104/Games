/**
 * Tongue Elevator Game
 * Elevator moves up as tongue elevation increases
 */
...
const ELEVATION_TARGET = 0.8; // 80% elevation for 3 stars
...
export function TongueElevatorGame({ onBack, onComplete, requiredRounds = TOTAL_ROUNDS }: Props) {
  ...
  const elevatorY = useRef(new Animated.Value(screenHeight * 0.7)).current;

  // Update tongue elevation and elevator position
  useEffect(() => {
    ...
    if (visible && elevation > 0) {
      ...
      setElevationHistory(prev => [...prev.slice(-50), elevation]); // Keep last 50 readings

      // Map elevation (0-1) to elevator Y position (bottom to top)
      const minY = screenHeight * 0.7; // Bottom position
      const maxY = screenHeight * 0.2; // Top position
      const targetY = minY - (elevation * (minY - maxY));
  ...
  }, [tongueElevation, isTongueVisible, isDetecting, gameState, screenHeight]);
  ...
  const startCalibration = useCallback(() => {
    ...
    setElevationHistory([]);
    setTimeElapsed(0);
    elevatorY.setValue(screenHeight * 0.7);

    if (currentRound === 1) {
      speak(
        'Welcome to Tongue Elevator! Lift your tongue to make the elevator go up. ' +
        'The higher you lift your tongue, the higher the elevator goes! Show your face to the camera to start!'
      );
    } else {
      speak(`Round ${currentRound}! Show your face to the camera to start!`);
    }
  }, [currentRound, screenHeight]);
  ...
  const endRound = useCallback(() => {
    ...
    const averageElevation = elevationHistory.length > 0
      ? elevationHistory.reduce((sum, e) => sum + e, 0) / elevationHistory.length
      : 0;

    let stars = 0;
    if (maxElevation >= ELEVATION_TARGET && averageElevation >= 0.5) {
      stars = 3;
    } else if (maxElevation >= ELEVATION_TARGET * 0.7) {
      stars = 2;
    } else if (maxElevation >= ELEVATION_TARGET * 0.5) {
      stars = 1;
    }
  ...
  }, [currentRound, maxElevation, elevationHistory, timeElapsed, requiredRounds, startCalibration]);
  ...
  const finishGame = useCallback(async () => {
    ...
    const averageMaxElevation = roundResults.reduce((sum, r) => sum + r.maxElevation, 0) / roundResults.length;
    const accuracy = Math.round(averageMaxElevation * 100);
    ...
    try {
      ...
      const result = await logGameAndAward({
        type: 'tongue-elevator',
        correct: totalStars,
        total: requiredRounds * 3,
        accuracy,
        xpAwarded,
        skillTags: ['tongue-elevation', 'oral-motor', 'tongue-control', 'sustained-movement'],
        meta: {
          totalRounds: requiredRounds,
          averageMaxElevation,
          totalStars,
    ...
    <SafeAreaView style={styles.container}>
      ...
      {gameState === 'playing' && (
        <View style={styles.gameArea}>
          {/* Elevator shaft */}
          <View style={[styles.shaft, { left: screenWidth / 2 - 40 }]} />
          ...
          >
            <View style={styles.elevatorBody}>
              <Text style={styles.elevatorEmoji}>🏢</Text>
            </View>
          </Animated.View>

          {/* Elevation display */}
          <View style={styles.elevationDisplay}>
            <Text style={styles.elevationLabel}>Tongue Elevation</Text>
            ...
            <View style={styles.progressBar}>
              <View
                style={[
                  styles.progressFill,
                  {
                    width: `${Math.min(100, currentElevation * 100)}%`,
                  },
                ]}
              />
            </View>
