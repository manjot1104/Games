/**
 * Roof Tap Game
 * Child taps roof of mouth with tongue (detect elevation spike)
 */
...
interface RoundResult {
  round: number;
  stars: number;
  taps: number; // Number of successful roof taps
  timeElapsed: number;
}

const TOTAL_ROUNDS = 5;
const ROUND_TIME_MS = 20000; // 20 seconds per round
const TAP_THRESHOLD = 0.8; // 80% elevation for tap
const TAP_COOLDOWN = 800; // 0.8 seconds cooldown between taps
...
export function RoofTapGame({ onBack, onComplete, requiredRounds = TOTAL_ROUNDS }: Props) {
  ...
  const roofGlow = useRef(new Animated.Value(0)).current;
  const lastTapTime = useRef(0);
  const wasAboveThreshold = useRef(false);

  // Update tongue elevation tracking and detect taps
  useEffect(() => {
    ...
    if (visible && elevation > 0) {
      ...
      // Detect tap: elevation spike above threshold, then decrease
      const now = Date.now();
      const isAboveThreshold = elevation >= TAP_THRESHOLD;
      const wasAbove = wasAboveThreshold.current;

      // Detect tap: went from below to above threshold (spike)
      if (isAboveThreshold && !wasAbove && (now - lastTapTime.current) > TAP_COOLDOWN) {
        lastTapTime.current = now;
        setTaps(prev => prev + 1);

        // Animate roof tap
        Animated.sequence([
          Animated.timing(roofGlow, {
            toValue: 1,
            duration: 150,
      ...
      setLastElevation(elevation);
    } else {
      wasAboveThreshold.current = false;
    }
  }, [tongueElevation, isTongueVisible, isDetecting, gameState]);
  ...
  const startCalibration = useCallback(() => {
    ...
    if (currentRound === 1) {
      speak(
        'Welcome to Roof Tap! Tap the roof of your mouth with your tongue. ' +
        'Lift your tongue up high to tap the roof! Show your face to the camera to start!'
      );
    } else {
  ...
  const startCountdown = useCallback(() => {
    ...
    countdownRef.current = setInterval(() => {
      setCountdown(prev => {
        ...
        speak(prev - 1 === 0 ? 'Go! Tap the roof!' : String(prev - 1));
        return prev - 1;
      });
    }, 1000);
  }, []);
  ...
  }, [currentRound, taps, timeElapsed, requiredRounds, startCalibration]);
  ...
  const finishGame = useCallback(async () => {
    ...
    const totalTaps = roundResults.reduce((sum, r) => sum + r.taps, 0);
    const accuracy = Math.round((totalTaps / (requiredRounds * 8)) * 100);
    ...
    try {
      ...
      const result = await logGameAndAward({
        ...
        accuracy,
        xpAwarded,
        skillTags: ['tongue-elevation', 'oral-motor', 'tongue-control', 'rapid-movement'],
        meta: {
          totalRounds: requiredRounds,
          totalTaps,
          totalStars,
  ...
  const glowOpacity = roofGlow.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 0.8],
  });
    ...
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.centerContent}>
          <Text style={styles.instructionText}>
            {jawError || !hasCamera
              ? 'Waiting for camera...'
              : isDetecting
              ? 'Great! Now get ready to tap the roof!'
      ...
      {gameState === 'playing' && (
        <View style={styles.gameArea}>
          {/* Roof indicator at top */}
          <Animated.View
            style={[
              styles.roofContainer,
              {
          ...
          >
            ...
            />
            <Text style={styles.roofText}>🏠 Roof of Mouth</Text>
          </Animated.View>
          ...
          <View style={styles.elevationIndicator}>
            <Text style={styles.elevationLabel}>
              {currentElevation >= TAP_THRESHOLD ? '⭐ Tap!' : 'Lift your tongue higher'}
            </Text>
            <View style={styles.progressBar}>
              <View
                style={[
                  styles.progressFill,
                  {
                    width: `${Math.min(100, (currentElevation / TAP_THRESHOLD) * 100)}%`,
                    backgroundColor: currentElevation >= TAP_THRESHOLD ? '#4CAF50' : '#FFA500',
                  },
                ]}
              />
            </View>
